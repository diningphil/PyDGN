...
class Optimizer(EventHandler):
    ...
    configuration file.
    """
    def __init__(self, model, optimizer_class_name, **kwargs):
        super().__init__()
    ...
    def on_training_batch_start(self, state):
        """
        Zeroes the gradient at the start of each (mini-)batch
    ...
    def on_training_batch_end(self, state):
        """
        Optimized the model at the end of each (mini-)batch
        :param state: the shared State object
        """
        self.optimizer.step()
