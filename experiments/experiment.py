...
class Experiment:
    ...
    def _create_model(self, dim_features, dim_target, predictor_classname, config):
        """
        Instantiates a model that implements a fixed interface in models.predictors
        :param dim_features: input features
        ...
        """
        return s2c(self.model_config.model)(dim_features=dim_features,
                                            dim_target=dim_target,
                                            predictor_class=s2c(predictor_classname),
                                            config=config)

    def create_supervised_model(self, dim_features, dim_target):
        """ Instantiates a supervised model """
        predictor_classname = self.model_config.supervised_config['predictor']
        return self._create_model(dim_features, dim_target, predictor_classname, self.model_config.supervised_config)

    def create_supervised_predictor(self, dim_features, dim_target):
        """ Directly instantiates a supervised predictor for semi-supervised tasks """
        return s2c(self.model_config.supervised_config['predictor'])(dim_features=dim_features,
                                                                     dim_target=dim_target,
                                                                     config=self.model_config.supervised_config)


    def create_unsupervised_model(self, dim_features, dim_target):
        """ Instantiates an unsupervised model """
        predictor_classname = self.model_config.unsupervised_config['predictor']
        return self._create_model(dim_features, dim_target, predictor_classname, self.model_config.unsupervised_config)

    def create_incremental_model(self, dim_features, dim_target, depth, prev_outputs_to_consider):
        """
        ...
        return self._create_model(dim_features, dim_target, predictor_classname, self.model_config.layer_config)

    def _create_wrapper(self, config, model, wrapper_classname, device):
        """
    ...
    def create_supervised_wrapper(self, model):
        ...
        return self._create_wrapper(self.model_config.supervised_config, model, wrapper_classname, device)
    ...
    def create_unsupervised_wrapper(self, model):
        ...
        return self._create_wrapper(self.model_config.unsupervised_config, model, wrapper_classname, device)
    ...
    def create_incremental_wrapper(self, model):
        ...
        return self._create_wrapper(self.model_config.layer_config, model, wrapper_classname, device)
