...
class SemiSupervisedTask(Experiment):
    ...
    def run_valid(self, dataset_getter, logger):
        ...
        shuffle = (
            unsupervised_config["shuffle"] if "shuffle" in unsupervised_config else True
        )
        ...
        shuffle = (
            supervised_config["shuffle"] if "shuffle" in supervised_config else True
        )
        ...
        model = self.create_supervised_model(
            dim_node_features=embedding_size, dim_edge_features=0, dim_target=dim_target
        )
